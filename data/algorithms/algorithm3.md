...
5:\ \quad \text{Compute } \mathbf{A} \text{ and } \mathbf{b} \text{ and solve} \\
   \qquad\qquad \left\{ \begin{array}{l} \mathbf{A}\,d\mathbf{q} = \mathbf{b} + \mathbf{H}_a^T \boldsymbol{\lambda}_a \\ \text{subject to} \\ \displaystyle\min_{\boldsymbol{\lambda}_a} \tfrac{1}{2}(\boldsymbol{\delta}_e(\mathbf{q}^{i-1}) + \mathbf{H}_e d\mathbf{q}) - \textcolor{green}{\mathbf{y}_e})^2 \end{array} \right. & (20) \\
