...
Fill the fields below and click the python button to run the resampling script.

1. **Input Filename**: you need to provide the path to the input `.txt` file containing the original mesh.
2. **Output Filename**: the path to the output `.txt` file, where the resampled mesh will be saved. 
3. **Number of Points**: the desired number of points in the resampled mesh. 

Either provide an absolute path, or a relative path from the current working directory, or only the filename if it is in the current working directory `/data/meshes/legs/`. 
...
#python-button(file="assets/labs/modules/beam_resampler.py", pyargs=["InputLegFilename", "OutputLegFilename", "NumPoints"])

:::: highlight
#icon("info") **Good to know**:
...
- The two first frames correspond to the part of the leg that is fixed to the motor, we keep them as it is and resample only the rest of the frames.
::::
