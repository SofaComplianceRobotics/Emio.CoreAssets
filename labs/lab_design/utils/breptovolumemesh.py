...
def generateMeshAndExport(filename, meshSizeFactor, display):
    ...
    # Here we generate the volume mesh and export it to a vtk file
    gmsh.model.mesh.generate(3)
    gmsh.write(dirpath + filename + '.vtk')
...
if __name__ == '__main__':
    """ Mesh Generation
     Generates a 3D mesh (tetrahedrons) from a ".step" file given by argument. 
     The given mesh filename should be in the 'data/meshes/legs' directory. The generated mesh will be added to the same
     directory.
    """

    parser = argparse.ArgumentParser(prog=sys.argv[0],
                                     description='Generates a 3D mesh (tetrahedrons) from a ".brep" file.')
